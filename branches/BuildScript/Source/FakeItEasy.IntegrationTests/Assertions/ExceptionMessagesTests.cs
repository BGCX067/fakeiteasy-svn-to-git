...
"));
        ...
        public void Should_be_able_to_assert_on_void_calls_from_configuration()
        {
            ...
            var thrown = Assert.Throws<ExpectationException>(() =>
                A.CallTo(() => foo.Bar(A<object>.Ignored, A<string>.That.StartsWith("lorem"))).MustHaveHappened(repeat => repeat > 1));

            Assert.That(thrown.Message, Is.EqualTo(@"

  Assertion failed for the following call:
    'FakeItEasy.Tests.IFoo.Bar(<Ignored>, <Starts with ""lorem"">)'
  Expected to find it the number of times specified by the predicate 'repeat => (repeat > 1)' but found it #1 times among the calls:
...
"));
        ...
        public void Should_be_able_to_assert_on_function_calls_from_configuration()
        {
            ...
            // Assert
            var thrown = Assert.Throws<ExpectationException>(() =>
                A.CallTo(() => foo.Baz(A<object>.Ignored, A<string>.That.StartsWith("lorem"))).MustHaveHappened(repeat => repeat > 1));
