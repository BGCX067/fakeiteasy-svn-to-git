...
namespace FakeItEasy.Tests
{
    ...
    public class FakeExtensionsTests
    {
        ...
        public void ReturnsNull_should_set_call_returns_with_null_on_configuration()
        {
            ...
            A.CallTo(() => config.Returns(A<string>.That.IsNull())).MustHaveHappened();
        }
        ...
        [Test]
        public void MustHaveHappened_should_call_MustHaveHappened_with_repeat_of_at_least_one()
        {
            // Arrange
            var asserts = A.Fake<IAssertConfiguration>();

            // Act
            FakeExtensions.MustHaveHappened(asserts);
            
            // Assert
            var expressionMatcher = A<Expression<Func<int, bool>>>.That.Matches(x => x.Compile()(1) && x.Compile()(10));
            A.CallTo(() => asserts.MustHaveHappened(expressionMatcher)).MustHaveHappened(repeat => repeat == 1);
        }
    }
}
