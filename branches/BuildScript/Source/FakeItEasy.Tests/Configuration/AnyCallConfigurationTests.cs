namespace FakeItEasy.Tests.Configuration
{
    ...
    public class AnyCallConfigurationTests
    {
        ...
        [Test]
        public void MustHaveHappened_delegates_to_configuration_produced_by_factory()
        {
            // Arrange
            Expression<Func<int, bool>> repeat = x => x == 1;

            var factoryConfig = this.StubVoidConfig();

            // Act
            this.configuration.MustHaveHappened(repeat);

            // Assert
            A.CallTo(() => factoryConfig.MustHaveHappened(repeat)).MustHaveHappened();
        }
