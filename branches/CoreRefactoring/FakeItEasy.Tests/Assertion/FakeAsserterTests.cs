...
namespace FakeItEasy.Tests.Assertion
{
    ...
    public class FakeAsserterTests
    {
        ...
        {
            return new FakeAsserter(this.calls);
        }
...
"));
  ...
  Expected to find it #2 times but no calls were made to the fake object."));
        ...
        [Test]
        public void Exception_message_should_truncate_calls_list_when_more_than_a_hundred_call_lines_are_printed()
        {
            this.StubCalls(30);
            ...
            foreach (var call in this.calls)
            {
                Fake.Configure(call)
                    .CallsTo(x => x.ToString())
                    .Returns(x => "Fake call {0}".FormatInvariant(Guid.NewGuid()));
            }

            Fake.Configure(this.calls[18])
                .CallsTo(x => x.ToString())
                .Returns("Last call");

            var asserter = this.CreateAsserter();
            var message = this.GetExceptionMessage(() =>
                asserter.AssertWasCalled(x => false, "", x => false, "never"));
