    ...
    public class FakeConfigurationTests
    {
        ...
        protected virtual void OnSetUp()
        {
            ...
            this.expressionRuleFactory = x =>
                {
                    var result = new ExpressionCallRule(new ExpressionCallMatcher(x, A.Fake<ArgumentValidatorFactory>(), new MethodInfoManager()));
        ...
        public void WhenArgumentsMatches_should_call_UsePredicateToValidateArguments_on_built_rule()
        {
            ...
            var config = new FakeConfiguration<IFoo>(this.fake, builtRule);
        ...
        {
            var returnConfig = this.configuration.CallsTo(x => x.Bar(null, null));
            
            Func<ArgumentCollection, bool> predicate = x => true;
            
            returnConfig.WhenArgumentsMatch(x => true);
            var call = ExpressionHelper.CreateFakeCall<IFoo>(x => x.Bar(10, 10));
            Assert.That(this.ruleProducedByFactory.IsApplicableTo(call), Is.True);
        }
